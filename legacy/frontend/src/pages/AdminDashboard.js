...
const AdminDashboard = () => {
  ...
  const handleGalleryFormChange = (field, value) => {
    setGalleryForm({ ...galleryForm, [field]: value });
  };
  ...
  const createGalleryImage = async (e) => {
    e.preventDefault();
    setError('');
    setLoading(true);
    ...
    try {
      ...
      const hasImageFile = Boolean(galleryForm.imageFile);
      const hasImageUrl = Boolean(galleryForm.imageUrl.trim());
      ...
      if (hasImageFile) {
        ...
        galleryData.append('title', galleryForm.title);
        galleryData.append('image', galleryForm.imageFile);
        if (hasImageUrl) {
          galleryData.append('image_url', galleryForm.imageUrl.trim());
        }
      } else {
        galleryData = {
      ...
      setError(err.response?.data?.message || 'Failed to add gallery image');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="admin-dashboard">
      ...
      <div className="admin-content">
        ...
        {activeTab === 'gallery' && (
          <div className="tab-content">
            <h2>Gallery</h2>
            <div className="project-form-container" style={{ marginBottom: '24px' }}>
              <h3>Add Gallery Image</h3>
              <form onSubmit={createGalleryImage}>
                <div className="form-row">
                  <div className="form-group">
                    <label>Title *</label>
                    <input
                      ...
                      placeholder="e.g., Contemporary Office Lobby"
                      required
                    />
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => handleGalleryFormChange('imageUrl', e.target.value)}
                ...
                <div className="form-group">
                  ...
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => handleGalleryFormChange('imageFile', e.target.files?.[0] || null)}
                  />
                  {galleryForm.imageFile && (
                    <p className="upload-summary">Selected: {galleryForm.imageFile.name}</p>
                  )}
                </div>
                ...
                <div className="form-actions">
                  ...
                  <button type="button" className="cancel-btn" onClick={resetGalleryForm}>
                    Clear
                  </button>
                </div>
              </form>
