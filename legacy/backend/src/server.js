...
const projectMediaStorage = (!useCloudinary || !cloudinaryImageStorage) ? multer.diskStorage({
  ...
  _handleFile(req, file, callback) {
    const storage = file.mimetype.startsWith('video/') ? cloudinaryVideoStorage : cloudinaryImageStorage;
    return storage._handleFile(req, file, callback);
  },
  _removeFile(req, file, callback) {
    const storage = file.mimetype.startsWith('video/') ? cloudinaryVideoStorage : cloudinaryImageStorage;
    return storage._removeFile(req, file, callback);
  },
};

const galleryImageStorage = (!useCloudinary || !cloudinaryImageStorage) ? multer.diskStorage({
  destination: (req, file, callback) => {
    callback(null, imageUploadsPath);
  },
  filename: (req, file, callback) => {
...
app.post('/api/gallery', authenticateToken, (req, res) => {
  uploadGalleryImage(req, res, (uploadError) => {
    if (uploadError) {
      return res.status(400).json({ message: uploadError.message });
    }

    const { title, image_url } = req.body;
    const uploadedImageUrl = req.file ? toPublicMediaPath(req.file.path) : image_url;
    ...
    const newImage = {
      ...
      display_order: mockGallery.length > 0 ? Math.max(...mockGallery.map((g) => g.display_order || 0)) + 1 : 1,
      standalone: true,
    };
